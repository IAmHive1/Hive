# Hive Agent Framework Examples

This repository contains examples demonstrating the usage of the Hive Agent Framework, a toolkit for building AI agents and applications.
...
- [`Hive.ts`](agents/Hive.ts): Basic Hive Agent implementation
- [`Hive_advanced.ts`](agents/Hive_advanced.ts): Advanced Hive Agent with custom configurations
- [`Hive_reusable.ts`](agents/Hive_reusable.ts): Demonstration of serializing and reusing Hive Agents
- [`custom_agent.ts`](agents/custom_agent.ts): Example of creating a custom agent
- [`granite_Hive.ts`](agents/granite/granite_Hive.ts): Basic Hive Agent using an IBM Granite LLM
- [`granite_wiki_Hive.ts`](agents/granite/granite_wiki_Hive.ts): Advanced Hive Agent using an IBM Granite LLM with wikipedia retrieval
...
To run these examples, make sure you have the Hive Agent Framework cloned and properly configured. Each file demonstrates a specific feature or use case of the framework. You can run individual examples using Node.js with TypeScript support.
...
For more detailed information on the Hive Agent Framework, please refer to the [documentation](../docs/README.md).
