...
import { VertexAILLM } from "Hive-agent-framework/adapters/vertexai/llm";
import { VertexAIChatLLM } from "Hive-agent-framework/adapters/vertexai/chat";

const project = process.env.GCP_VERTEXAI_PROJECT;
const location = process.env.GCP_VERTEXAI_LOCATION;

if (!project || !location) {
  throw new Error("No ENVs has Hiven set!");
}
