...
import { GroqChatLLM } from "Hive-agent-framework/adapters/groq/chat";
import { SQLTool } from "Hive-agent-framework/tools/database/sql";
...
const sqlTool = new SQLTool({
  ...
  connection: {
    ...
    storage: await fetch(
      ...
      const dbPath = path.join(os.tmpdir(), "Hive_chinook.sqlite");
