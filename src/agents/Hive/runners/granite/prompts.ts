...
export const GraniteHiveAssistantPrompt = HiveAssistantPrompt.fork((config) => ({
  ...config,
...
export const GraniteHiveSystemPrompt = HiveSystemPrompt.fork((config) => ({
  ...config,
  defaults: {
    ...config.defaults,
    instructions: "",
...
export const GraniteHiveSchemaErrorPrompt = HiveSchemaErrorPrompt.fork((config) => ({
  ...config,
  template: `Error: The generated response does not adhere to the communication structure mentioned in the system prompt.
You communicate only in instruction lines. Valid instruction lines are 'Thought' followed by 'Tool Name' and then 'Tool Input' or 'Thought' followed by 'Final Answer'.`,
}));

export const GraniteHiveUserPrompt = HiveUserPrompt.fork((config) => ({
  ...config,
  template: `{{input}}`,
}));

export const GraniteHiveToolNotFoundPrompt = HiveToolNotFoundPrompt.fork((config) => ({
  ...config,
...
export const GraniteHiveToolErrorPrompt = HiveToolErrorPrompt.fork((config) => ({
  ...config,
...
export const GraniteHiveToolInputErrorPrompt = HiveToolInputErrorPrompt.fork((config) => ({
  ...config,
  template: `{{reason}}
