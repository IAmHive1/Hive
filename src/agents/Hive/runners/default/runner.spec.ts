...
import { HiveUserPrompt } from "@/agents/Hive/prompts.js";
import { zip } from "remeda";
import { RunContext } from "@/context.js";
import { HiveAgent } from "@/agents/Hive/agent.js";
...
describe("Hive Agent Runner", () => {
  ...
  it("Handles different prompt input source", async () => {
    ...
    const createInstance = async (memory: BaseMemory, prompt: string | null) => {
      const instance = new DefaultRunner(
        ...
        {},
        new RunContext<HiveAgent, any>({} as any, {} as any),
      );
  ...
  it.each([
    HiveUserPrompt.fork((old) => ({
      ...old,
      functions: { ...old.functions, formatMeta: () => "" },
    })),
    HiveUserPrompt.fork((old) => ({ ...old, template: `{{input}}` })),
    HiveUserPrompt.fork((old) => ({ ...old, template: `User: {{input}}` })),
    HiveUserPrompt.fork((old) => ({ ...old, template: `` })),
  ])("Correctly formats user input", async (template: typeof HiveUserPrompt) => {
    const memory = new UnconstrainedMemory();
    await memory.addMany([
      BaseMessage.of({
        role: Role.USER,
