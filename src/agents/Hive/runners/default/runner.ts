...
import { BaseRunner, HiveRunnerLLMInput, HiveRunnerToolInput } from "@/agents/Hive/runners/base.js";
import type {
...
export class DefaultRunner extends BaseRunner {
  ...
  async llm({ signal, meta, emitter }: HiveRunnerLLMInput): Promise<HiveAgentRunIteration> {
  ...
  async tool({ state, signal, meta, emitter }: HiveRunnerToolInput) {
  ...
  protected async initMemory({ prompt }: HiveRunInput): Promise<BaseMemory> {
    ...
    const prevConversation = [...history.messages, this.renderers.user.message({ prompt })]
      ...
      .map((message) => {
        if (message.role === Role.USER) {
          ...
          const text = isEmpty
            ? (this.templates?.userEmpty ?? HiveUserEmptyPrompt).render({})
            : (this.templates?.user ?? HiveUserPrompt).render({
                input: message.text,
                meta: {
                  ...message?.meta,
  ...
  get templates(): HiveAgentTemplates {
    ...
    return {
      system: customTemplates.system ?? HiveSystemPrompt,
      assistant: customTemplates.assistant ?? HiveAssistantPrompt,
      user: customTemplates.user ?? HiveUserPrompt,
      userEmpty: customTemplates.userEmpty ?? HiveUserEmptyPrompt,
      toolError: customTemplates.toolError ?? HiveToolErrorPrompt,
      toolInputError: customTemplates.toolInputError ?? HiveToolInputErrorPrompt,
      toolNoResultError: customTemplates.toolNoResultError ?? HiveToolNoResultsPrompt,
      toolNotFoundError: customTemplates.toolNotFoundError ?? HiveToolNotFoundPrompt,
      schemaError: customTemplates.schemaError ?? HiveSchemaErrorPrompt,
    };
  }
  ...
  protected createParser(tools: AnyTool[]) {
    ...
    const parser = new LinePrefixParser<HiveParserInput>(
      {
        thought: {
          prefix: "Thought:",
