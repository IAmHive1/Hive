...
import type { HiveCallbacks } from "@/agents/Hive/types.js";
...
import type { HiveAgent } from "@/agents/Hive/agent.js";
...
export function createTelemetryMiddleware() {
  return (context: GetRunContext<RunInstance, unknown>) => {
    ...
    if (instance instanceof BaseAgent) {
      prompt = (runParams as Parameters<HiveAgent["run"]>)[0].prompt;
    }
    ...
    const partialUpdateEventName: keyof HiveCallbacks = "partialUpdate";
    ...
    emitter.match(
      ...
      (data: InferCallbackValue<HiveCallbacks[typeof successEventName]>) => {
