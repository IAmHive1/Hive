...
Our Hive Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new HiveAgent({
  llm: new OllamaChatLLM(), // for more explore 'Hive-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Hive-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Hive-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Hive_advanced.ts) to see how to modify them.
