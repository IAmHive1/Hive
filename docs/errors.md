...
> [!TIP]
>
> Location within the framework `Hive-agent-framework/error`.
...
All errors thrown within the Hive Agent Framework extend from the base [FrameworkError](/src/errors.ts) class, which itself extends Node.js's native [AggregateError](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/AggregateError).
...
> [!TIP]
>
> The `explain()` method is particularly useful for returning a simplified, human-readable error message to an LLM, as used by the Hive Agent.
...
```ts
import { DynamicTool, ToolError } from "Hive-agent-framework/tools/base";
import { FrameworkError } from "Hive-agent-framework/errors";
import { z } from "zod";
